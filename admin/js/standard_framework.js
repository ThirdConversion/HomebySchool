...
jQuery(document).ready(function($){
	...
	$('li#toplevel_page_standardframework a').live('click', function(){
		scroll(0,0);
	});
	
	$('li#toplevel_page_standardframework li.wp-first-item').remove();
	...
	function change_tab( id ){
		...
		$('#import-export').hide(); // added for ie7/8 issues
		if(id == '#import-export'){
			$('button#standard_save').fadeOut();
		} else {
			$('button#standard_save').fadeIn();
		}
	...
	$('#standard_framework input, #standard_framework select, #standard_framework textarea[class!=click-copy][class!=standard_import_code]').change(function(e){
		$('#button-zone').animate({ 
			backgroundColor: '#555',
			borderLeftColor: '#555',
