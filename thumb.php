...
$src = get_request("src", "");
if($src == "" || strlen($src) <= 3) {
	displayError(__("no image specified",'standardtheme'));
}
...
if(!valid_src_mime_type($mime_type)) {
	displayError(__("Invalid src mime type: ",'standardtheme') .$mime_type);
}

// check to see if GD function exist
if(!function_exists('imagecreatetruecolor')) {
	displayError(__("GD Library Error: imagecreatetruecolor does not exist",'standardtheme'));
}
...
if(strlen($src) && file_exists($src)) {
	...
	if($image === false) {
		displayError(__('Unable to open image : ','standardtheme') . $src);
	}
	...
	if(strlen($src)) {
		displayError(sprintf(__("image %s not found",'standardtheme'), $src));
	} else {
		displayError(__("no source specified",'standardtheme'));
	}
...
function show_image($mime_type, $image_resized, $cache_dir) {
	...
	displayError(__("error showing image",'standardtheme'));
...
function get_document_root ($src) {
	...
	displayError(sprintf(__('file not found %s','standardtheme'), $src));
